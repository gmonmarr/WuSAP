...
describe('authService', () => {
  ...
  it('should reject if user is not found', async () => {
    ...
    await expect(authService.loginUser('notfound@site.com', 'password'))
  ...
  it('should reject if password does not match', async () => {
    ...
    await expect(authService.loginUser('test@site.com', 'wrong'))
      .rejects.toThrow('Contraseña incorrecta');
  });

  it('should reject if user is blocked', async () => {
    const fakeUser = { ...defaultFakeUser, ISBLOCKED: true, BLOCKREASON: 'Policy' };
    fakeStatement.exec.mockImplementation((params, cb) => cb(null, [fakeUser]));
    await expect(authService.loginUser('blocked@site.com', 'pass'))
  ...
  it('should reject if user is not active', async () => {
    ...
    fakeStatement.exec.mockImplementation((params, cb) => cb(null, [fakeUser]));
    await expect(authService.loginUser('inactive@site.com', 'pass'))
  ...
  it('should register a new user successfully', async () => {
    ...
    const result = await authService.registerUser(1, 'New', 'User', 'new@site.com', '123', 'pwd', 'employee', 2);
    expect(result.success).toBe(true);
  ...
  it('should not register a user that already exists', async () => {
    fakeStatement.exec.mockImplementation((params, cb) => cb(null, [{ EMAIL: 'taken@site.com' }]));
    await expect(authService.registerUser(1, 'Old', 'User', 'taken@site.com', '123', 'pwd', 'employee', 2))
  ...
  it('should rollback on error during registration', async () => {
    ...
    await expect(authService.registerUser(1, 'New', 'User', 'fail@site.com', '123', 'pwd', 'employee', 2))
