...
describe('inventoryService', () => {
  ...
  it('getAllInventory returns rows', async () => {
    ...
    expect(fakeConn.exec).toHaveBeenCalledWith(expect.stringContaining('SELECT'), expect.any(Function));
  ...
  it('getWarehouseProducts returns correct products', async () => {
    ...
    expect(result).toBe(rows);
    expect(fakeConn.exec).toHaveBeenCalledWith(expect.stringContaining('INNER JOIN'), expect.any(Function));
    expect(pool.release).toHaveBeenCalledWith(fakeConn);
  });
...
fakeConn.exec.mockImplementation((sql, cb) => cb(null, [{ INVENTORYID: 42 }]));
...
it('assignInventoryToStore inserts if not exists', async () => {
  ...
  it('editInventory works (and calls log)', async () => {
    ...
    fakeConn.prepare.mockImplementation((sql, cb) => cb(null, { exec: (params, cb2) => cb2(null) }));
