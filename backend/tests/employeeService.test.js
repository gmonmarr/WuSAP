...
describe('employeeService', () => {
  ...
  it('should update employee with provided fields', async () => {
    ...
    const result = await employeeService.updateEmployee(employeeID, employeeData, updatedByID);
  ...
  it('should throw error if invalid role is provided', async () => {
    ...
    await expect(employeeService.updateEmployee(1, badData, 2))
      .rejects.toThrow('El rol debe ser admin, manager, sales, owner o warehouse_manager');
  ...
  it('should rollback on SQL error', async () => {
    ...
    fakeConn.exec.mockImplementationOnce(() => { throw new Error('DB error'); });
    await expect(employeeService.updateEmployee(1, { name: 'Test' }, 99))
