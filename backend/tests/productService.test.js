...
describe('productService', () => {
  ...
  it('getAllProducts returns all products', async () => {
    const rows = [{ PRODUCTID: 1, NAME: 'A' }, { PRODUCTID: 2, NAME: 'B' }];
    fakeConn.exec.mockImplementation((sql, cb) => cb(null, rows));

    const result = await getAllProducts();
    expect(result).toEqual(rows);
    expect(fakeConn.exec).toHaveBeenCalledWith(expect.stringContaining('Products'), expect.any(Function));
  ...
  it('getActiveProductsService returns only active products', async () => {
    ...
    expect(result).toEqual(rows);
    expect(fakeConn.exec).toHaveBeenCalledWith(expect.stringContaining('discontinued = FALSE'), expect.any(Function));
    expect(pool.release).toHaveBeenCalledWith(fakeConn);
  });
    ...
    fakeConn.prepare.mockImplementation((sql, cb) => cb(null, { exec: prepareExec }));

    // exec() calls: 
    // 1. SELECT CURRENT_IDENTITY_VALUE() (returns [{ PRODUCTID: 99 }])
    fakeConn.exec
        .mockImplementationOnce((sql, cb) => cb(null, [{ PRODUCTID: 99 }]));
  ...
  it('updateProduct updates fields and logs', async () => {
    ...
    const result = await updateProduct(7, { name: 'X', suggestedPrice: 1.11 }, 5);
  ...
  it('updateProduct returns false if no fields', async () => {
    fakeConn.exec.mockImplementationOnce((sql, params, cb) => cb(null, [{ PRODUCTID: 7 }]));
  ...
  it('updateProduct throws if product not found', async () => {
    ...
    await expect(updateProduct(101, { name: 'No' }, 3)).rejects.toThrow('Product not found');
    expect(fakeConn.rollback).toHaveBeenCalled();
    expect(pool.release).toHaveBeenCalledWith(fakeConn);
  });
  ...
  it('deleteProduct deletes product and logs', async () => {
    ...
    fakeConn.exec
      .mockImplementationOnce((sql, params, cb) => cb(null, [{ PRODUCTID: 9, NAME: 'ToDel' }]))
      .mockImplementationOnce((sql, params, cb) => cb(null, [{ USAGE_COUNT: 0 }]))
  ...
  it('deleteProduct errors if product is used', async () => {
    fakeConn.exec
      .mockImplementationOnce((sql, params, cb) => cb(null, [{ PRODUCTID: 2, NAME: 'CantDel' }]))
      .mockImplementationOnce((sql, params, cb) => cb(null, [{ USAGE_COUNT: 1 }]));
