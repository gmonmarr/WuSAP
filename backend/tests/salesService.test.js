...
describe('salesService', () => {
  ...
  it('getAllSales returns sales with employee names', async () => {
    const rows = [
      { saleID: 1, saleDate: '2024-01-01', employeeID: 5, employeeName: 'Alice', saleTotal: 99 }
    ];
  ...
  it('deleteSale refunds inventory, logs, and deletes sale/items', async () => {
    ...
    fakeConn.exec
      // Get old sale
      .mockImplementationOnce((sql, params, cb) => cb(null, [{ SALETOTAL: 42, SALEDATE: '2024-01-01' }]))
      // Get old sale items
      .mockImplementationOnce((sql, params, cb) => cb(null, [
        { SALEITEMID: 10, INVENTORYID: 4, QUANTITY: 2 }
      ]))
      // Delete SaleItems
      .mockImplementationOnce((sql, params, cb) => cb(null))
      // Delete Sale
