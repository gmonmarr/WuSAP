...
CREATE TABLE WUSAP.Orders (
  ...
  status NVARCHAR(50), -- estatus del producto ["Pendiente", "Aprobada", "Confirmada", "Entregada", "Cancelada"]
...
CREATE TABLE WUSAP.OrderHistory (
  ...
  orderID INTEGER, -- orderitem que se hacer referencia
...
CREATE TABLE WUSAP.SaleItems (
  ...
  inventoryID INTEGER, -- referencia a inventario
  ...
  FOREIGN KEY (inventoryID) REFERENCES WUSAP.Inventory(inventoryID)
);
...
CREATE TABLE WUSAP.TableLogs (
  ...
  comment NVARCHAR(500), -- Optional comment describing the change
  timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (employeeID) REFERENCES WUSAP.Employees(employeeID)
);
